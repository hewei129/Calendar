...
//    protected Boolean mDrawRect;              // 圆角还是圆形
    ...
    public SimpleMonthAdapter.CalendarDay mStartDate;          // 入住日期
    public SimpleMonthAdapter.CalendarDay mEndDate;            // 退房日期
    ...
    protected void drawMonthCell(Canvas canvas) {
        ...
        while (day <= mNumCells) {
            ...
            if (mHasToday && (mToday == day)) {
                isToady = true;
                canvas.drawText(resources.getString(R.string.label_now_time), x, getTextYCenter(mDayTextPaint, y - DAY_SELECTED_RECT_SIZE / 2), mDayTextPaint);
            }
            // 已过去的日期
            ...
            if (mStartDate != null && cellCalendar.equals(mStartDate) && !mStartDate.equals(mEndDate)) {
                ...
                canvas.drawText(resources.getString(R.string.label_start_time), x, getTextYCenter(mDayTextPaint, y + DAY_SELECTED_RECT_SIZE / 2), mDayTextPaint);
                if(isToady) {
                    canvas.drawText(resources.getString(R.string.label_now_time), x, getTextYCenter(mDayTextPaint, y - DAY_SELECTED_RECT_SIZE / 2), mDayTextPaint);
                }
            }
            ...
            if (mEndDate != null && cellCalendar.equals(mEndDate) && !mStartDate.equals(mEndDate)) {
                ...
                drawDayBg(canvas, x, y, mSelectedDayBgPaint);
                mDayTextPaint.setColor(mSelectedDayTextColor);
                canvas.drawText(resources.getString(R.string.label_end_time), x, getTextYCenter(mDayTextPaint, y + DAY_SELECTED_RECT_SIZE / 2), mDayTextPaint);
            }
